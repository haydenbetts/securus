...
const { loadFacilities } = require('./firebase_etl');
...
const main = async (today = moment(new Date()).format("MM_DD_YYYY"), load = true) => {
  ...
  //   }
  // }
  const transformed = transformFacilities(FILE);
  if (load) {
    await loadFacilities(transformed);
  }
  return transformed;
};
